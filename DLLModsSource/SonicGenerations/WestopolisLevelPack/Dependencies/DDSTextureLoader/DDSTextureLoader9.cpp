...
// This file has been modified to add support for ATI1/ATI2.
...
// ATI1/ATI2 definitions.
#define D3DFMT_ATI1  (D3DFORMAT) MAKEFOURCC('A', 'T', 'I', '1')
#define D3DFMT_ATI2  (D3DFORMAT) MAKEFOURCC('A', 'T', 'I', '2')

#define D3DFMT_BC4U  (D3DFORMAT) MAKEFOURCC('B', 'C', '4', 'U')
#define D3DFMT_BC5U  (D3DFORMAT) MAKEFOURCC('B', 'C', '5', 'U')
...
namespace
{
        ...
        bool generateMipsIfMissing) noexcept
    {
        ...
        // Mipmaps smaller than 4x4 won't have enough space allocated for ATI1/ATI2 textures,
        // so mipmap count is going to be clamped to avoid memory corruptions.
        if (fmt == D3DFMT_ATI1 || fmt == D3DFMT_ATI2)
        {
            UINT iTmpWidth = iWidth;
            UINT iTmpHeight = iHeight;
            UINT iTmpMipCount = 1;

            while (iTmpWidth > 4 && iTmpHeight > 4)
            {
                if (iTmpWidth > 4)
                    iTmpWidth >>= 1;
            ...
            if (iTmpMipCount < iMipCount)
            {
                iMipCount = iTmpMipCount;
            }
        }

        // ATI2 has red & green channels swapped in D3D9.
        // Data is going to be swizzled to overcome this behavior.
        if (fmt == D3DFMT_ATI2)
        {
            UINT64* pBits = (UINT64*)bitData;
            UINT64* pBitsEnd = (UINT64*)(bitData + bitSize);

            while (pBits < pBitsEnd)
            {
                UINT64 Red = pBits[0];
                UINT64 Green = pBits[1];
        ...
        if (header->flags & DDS_HEADER_FLAGS_VOLUME)
        {
            ...
            for (UINT i = 0; i < iMipCount; ++i)
            {
                ...
                {
                    // ATI1/ATI2 textures are not recognized by D3D9 runtime,
                    // so they are going to have incorrect pitch values.
                    if (fmt == D3DFMT_ATI1 || fmt == D3DFMT_ATI2)
                    {
                        LockedBox.RowPitch = RowBytes;
                    }
